...
hypernum = Hypernum(
        data_path = r"Datasets/cv17",
        audio_file = r"cv17en/audio_npy_resample_en",
...
model = SpeechReModel.load_from_checkpoint(r"/root/autodl-tmp/checkpoints/cv17-epoch=06-F1-vote投票后-三元组F1=0.17-关系学习率大十倍.ckpt",hypernum = hypernum)
...
checkpoint_callback = ModelCheckpoint(dirpath="/root/autodl-tmp/checkpoints", 
                                      filename='cv17-{epoch:02d}-F1-vote{投票后-三元组F1:.2f}',
                                      save_top_k=1, 
                                      monitor='投票后-三元组F1',
                                      mode = 'max',
